...
export const renderMarkdownToHTML = (text) => {
  ...
  html = html.replace(/^### (.+)$/gm, '<h4 class="font-bold text-sm mt-3 mb-1 text-slate-200">$1</h4>');
  html = html.replace(/^## (.+)$/gm, '<h3 class="font-bold text-base mt-3 mb-1 text-slate-200">$1</h3>');
  html = html.replace(/^# (.+)$/gm, '<h2 class="font-bold text-lg mt-3 mb-1 text-white">$1</h2>');
  ...
  // Inline code
  html = html.replace(/`(.+?)`/g, '<code class="bg-slate-700/60 text-indigo-300 px-1.5 py-0.5 rounded text-[0.85em] font-mono">$1</code>');

  // Bullet lists: consecutive lines starting with "- "
  html = html.replace(/(^- .+$(\n|$))+/gm, (match) => {
    const items = match.trim().split('\n').map((line) => {
      const content = line.replace(/^- /, '');
      return `<li class="ml-4 list-disc text-slate-300">${content}</li>`;
    }).join('');
